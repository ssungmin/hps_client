...
def makeRow(rows):
      # print(row[54])
        ...
        hps_floor = rows[44]
        hps_poi = rows[45]
        airpress = str(rows[46])
        print(f"airpress1={airpress}")
        ...
        if btinfoCnt != "0" :
            for i in range(0 , int(btinfoCnt)) :
                ...
                ble_end = offset + 2
                #print(f"ble_end={ble_end}")

                btinfo.append({'btMACAddress':str(rows[btMACAddress]),'btSignalStrength':str(rows[btSignalStrength]), 'btDeviceName':str(rows[btDeviceName])})
        else :
        ...
        #print(f"magnetic end={magnetic_end}")
        ...
        airpress_ref_info=[] 
       # print(f"cell2_ref_info_end={cell2_ref_info_end}") 
       # print(f"action_type={action_type_flag}") 
            #print(f"action_type=6이아님") 
...
if __name__ == "__main__":
    ...
    try:
    #로컬
        dir_path="/Users/1111764/Documents/location/development/TIDC/TIDC/hps_client/parsing/sample"
        print (dir_path)
        print("%s 시작" % sys.argv[1])
