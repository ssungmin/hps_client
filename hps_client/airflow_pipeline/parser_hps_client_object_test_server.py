...
minio = fs.S3FileSystem(
     endpoint_override='10.1.1.132:9002',
     access_key='minioadmin',
     secret_key='minioadmin',
     scheme='http')
...
def ray_test(csv_file):
    ...
    #print(list_row[0]['collect_dt'])
    df = pd.DataFrame(list_row)
    
    table = pa.Table.from_pandas(df, schema=parquet_schema_new)   
    print(df)
   ...
   # pq.write_to_dataset(table ,"/Users/1111764/Documents/location/development/TIDC/TIDC/hps_client/parsing/HPS.parquet",compression='GZIP')
  ...
  with open(csv_file, 'r', encoding='utf-8', errors='replace') as file:
    ...
    for i, line in enumerate(lines):
        ...
        try: 
            ...
            logger.error(f"{str(i)} Parse fail ")
            pass
    #df = pd.DataFrame(list_row,c)
    #df = pd.DataFrame(list_row)
    table = pa.Table.from_pylist(list_row , schema=parquet_schema_new)
    print(table)
...
#dir_path = '/DATA/pms_app/PMS_DATA'
    ...
    dir_path=os.path.join(dir_path, yesterday)
    #print (dir_path)
