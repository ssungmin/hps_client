...
def folder_file_copy(copyPath: str, pastePath: str, dt: str):
    ...
    try :
       os.chdir(copyPath)
       os.system("sudo find . -name '*%s*' -exec cp -f {} --parents /DATA/pms_app/PMS_DATA/%s \;" % (dt,dt))
       #pastePath= (os.path.join(pastePath, dt))
       os.chdir(pastePath)
       os.system("sudo chown -R svcapp_su:svcapp %s \;" % (dt))
       # 파일 카피 후 소유자 변경 sudo chown -R svcapp_su:svcapp 20231024
...
def origin_rm( copyPath:str ,dt: str):
    ...
    try :
       os.chdir(copyPath)
       os.system('sudo find . -type f -name "*%s*" -exec rm {} \; ' % dt)
...
def target_directory_rm(dt):
    ...
    #datetime_result = dt.strtime( '%Y-%m-%d')
    rm_dt = dt - timedelta(8)
